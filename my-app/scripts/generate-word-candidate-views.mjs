...
const viewProofs = views.map((view) => {
  ...
  return {
    ...
    all_ninety_four_sealed_words_preserved_as_single_segments: wordSegments.length === 94
      && new Set(wordSegments.map((segment) => segment.reference_id)).size === 94,
...
const proof = {
  ...
  all_sealed_words_preserved_in_both_views: viewProofs.every((item) => item.all_ninety_four_sealed_words_preserved_as_single_segments),
